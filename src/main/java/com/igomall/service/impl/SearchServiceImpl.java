...
public class SearchServiceImpl implements SearchService {
	...
	@Override
	public Page<Course> search(String keyword, Lesson lesson, BigDecimal startPrice, BigDecimal endPrice, Course.OrderType orderType, Pageable pageable) {
		if (StringUtils.isEmpty(keyword)) {
			return new Page<>();
		}
		...
		FullTextEntityManager fullTextEntityManager = Search.getFullTextEntityManager(entityManager);
		QueryBuilder queryBuilder = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(Course.class).get();
		...
		if (lesson != null) {
			Query storeQuery = queryBuilder.phrase().onField("lesson.id").sentence(String.valueOf(lesson.getId())).createQuery();
		...
		FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(booleanJunction.createQuery(), Course.class);
	...
	public List<Lesson> searchLesson(String keyword) {
		...
		FullTextEntityManager fullTextEntityManager = Search.getFullTextEntityManager(entityManager);
		QueryBuilder queryBuilder = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(Lesson.class).get();
		...
		FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(query, Lesson.class);
